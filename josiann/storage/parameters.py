...
from josiann.moves import SetStretch
...
class MoveParameters:
    ...
    def __repr__(self) -> str:
        return '\t' + '\n\t'.join([f"{type(move).__name__} with probability {np.round(proba, 4)}"
    ...
    def set_bounds(self,
                   bounds: tuple[float, float] | Sequence[tuple[float, float]] | None) -> None:
        """
...
def check_base_parameters_core(T_0: float,
                               ...
                               tol: float,
                               x0: np.ndarray,
                               dtype: type[np.dtype] | type[np.number]) \
        -> tuple[float, float, int, int, float, np.ndarray]:
    # max iterations
    if max_iter < 0:
...
def check_base_parameters(args: Sequence | None,
                          x0: np.ndarray,
                          nb_walkers: int,
                          max_iter: int,
                          max_measures: int,
                          ...
                          suppress_warnings: bool,
                          detect_convergence: bool,
                          dtype: type[np.dtype] | type[np.number]) -> BaseParameters:
    """
    ...
    return BaseParameters(args=args,
                          x0=x0,
                          max_iter=max_iter,
...
def check_bounds(bounds: tuple[float, float] | Sequence[tuple[float, float]] | None,
                 x0: np.ndarray) -> None:
    """
