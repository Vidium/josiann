...
class ParallelTrace(Trace):
    ...
    def __repr__(self) -> str:
        return f"Parallel trace of {self.nb_iterations} iteration(s), {self.nb_walkers} parallel problem(s) and " \
    ...
    def plot_positions(self,
        ...
        titles = ["Costs"] + ['' for _ in range(self.nb_walkers - 1)] + \
                 ["Best cost evolution"] + ['' for _ in range(self.nb_walkers - 1)]

        for i in range(self.nb_dimensions):
            dimension_title = dimension_titles[i] if dimension_titles is not None else f"Dimensions #{i}"

            if walker_titles is not None:
                titles += [f'{dimension_title}<br>{walker_titles[w]}<br>(converged : {self.positions.converged[w]})'
                           for w in range(self.nb_walkers)]

            else:
                titles += [f'{dimension_title}<br>(converged : {self.positions.converged[w]})'
                           for w in range(self.nb_walkers)]

        fig = make_subplots(rows=self.nb_dimensions + 2,
        ...
        for w in trange(self.nb_walkers, leave=False):
            # -----------------------------------------------------------------
            # Cost trace
            fig.add_trace(go.Scatter(x=list(range(self.positions.nb_iterations + 1)),
            ...
            for d in range(self.nb_dimensions):
                ...
                if true_values is not None:
                    fig.add_trace(go.Scatter(x=[0, self.positions.nb_iterations],
